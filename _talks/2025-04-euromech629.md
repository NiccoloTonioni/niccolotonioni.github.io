---
title: "Data-driven modeling of near-wall turbulence using β-variational autoencoder, transformers, and adversarial loss"
collection: talks
type: "Conference talk"
permalink: /talks/2025-04-euromech629
venue: "Joint event Euromech Colloquium on Data-Driven Fluid Dynamics & 2nd ERCOFTAC Workshop on Machine Learning for Fluid Dynamics"
date: 2025-04-02
location: "London, GB"
---
<div style="text-align: left;">
  <p style="margin-left: auto; margin-right: auto; max-width: 800px;">
    Can machine learning help us understand and predict near-wall turbulence? In this talk, I presented a framework combining a β-variational autoencoder (β-VAE) for unsupervised feature extraction and a unidirectional transformer for temporal prediction of turbulent channel flows. Using minimal channel simulations at \(Re_\tau = 200\), we assess the framework’s ability to capture compact, interpretable representations and forecast flow dynamics. Validation includes energy spectra, quadrant analysis, and dynamical tools such as Lyapunov exponents and Poincaré maps.
  </p>
</div>
<figure style="text-align: center; margin-top: 15px;">
    <img src="/assets/images/euromech_architecture.png" alt="Prediction with VAE and Transformer" style="width: 400px; height: auto; border-radius: 5px;">
    <figcaption>Model's framework.</figcaption>
</figure>